...
graph TB
    ...
    subgraph Host ["☁️ AWS EC2 Instance"]
        Nginx
        Frontend["📦 Frontend Container (Nginx Static)"]
        Backend["📦 Backend Container (Node/Express API)"]
        Redis[("⚡ Redis Container (Cache & Sockets)")]
    end
    ...
    Nginx -->|Routes /api & /socket.io| Backend
    
    Backend <-->|Session Store & Sockets| Redis
    Backend <-->|Data Persistence| MongoDB
    Backend -->|Generates Presigned Upload URLs| S3
    Backend <-->|Token Sync & Webhooks| Stream
    Backend <-->|Session Verification| Clerk
...
Streamify is designed with a **decoupled Client-Server Architecture** optimized for high performance, ease of scaling, and security:

* **Client Tier:** A single-page application built on **React 19** and **Vite**. It interacts directly with **Clerk** for user identity management, **GetStream.io** for low-latency WebRTC audio/video call streaming, and **AWS S3** for direct file uploads (utilizing secure, short-lived presigned URLs generated by our API).
* **Reverse Proxy Tier:** **Nginx** handles secure SSL termination (via Let's Encrypt), static file serving, and acts as a gateway proxy to direct API requests (`/api`) and WebSocket connections (`/socket.io`) to the backend container.
* **Application Tier:** A **Node.js** & **Express** REST API. It handles application state, business logic (friends, posts, communities), generates authentication tokens for GetStream, and coordinates WebSocket communication using **Socket.io**.
* **Cache & Pub/Sub Tier:** A **Redis** container is used to manage real-time active user sockets, track active user presence, and handle communication sync across backend threads.
* **Database Tier:** A hosted **MongoDB Atlas** database serves as the source of truth for persistent user profiles, social relationships, posts, and call histories.
